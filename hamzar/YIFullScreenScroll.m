...
- (void)setStyle:(YIFullScreenScrollStyle)style
{
    if (style != _style) {
        ...
        // auto-adjust
        if (IS_FLAT_DESIGN && style == YIFullScreenScrollStyleDefault) {
...
- (CGFloat)additionalNavBarShiftForIOS7StatusBar
{
    ...
    if (IS_FLAT_DESIGN && _style == YIFullScreenScrollStyleDefault) return 0;
...
- (void)_layoutUIBarsWithDeltaY:(CGFloat)deltaY
{
    ...
    if (isNavigationBarExisting && _shouldHideNavigationBarOnScroll) {
        if (canLayoutUIBars) {
            ...
            if (IS_FLAT_DESIGN && _style == YIFullScreenScrollStyleDefault) {
